    ...
    def fit_inliers_auto_ensemble_auto_factor(self, step=0.05, import_from=None, plot_results=False):
        ...
        if import_from is None:
            for i in range(100, 205 ,new_step):
                ...
                f_results[f] = {'recall': len(results.get_false_negatives('Auto ensemble')), 'results': results}
        else:
            for i in range(100, 205 ,new_step):
                f = i/100
                try:
                    ...
                    f_results[f] = {'recall': results.recall('Auto ensemble'), 'results': results}
                except Exception as a:
                    print(f'{import_from}/results_f{f}.pickle file not found', a)
                    
        min_fn = max(values['recall'] for values in f_results.values())
